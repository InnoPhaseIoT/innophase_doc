...
Refer section  for the details. :ref:`E&D - Storage Data & File: Read/Write<storage-data-file-readwrite-2>`
...
2. where,
   ...
   b. [device]: Communication device ID with path.
      ...
      iii. If there are multiple Talaria TWO devices where each of them has their own peripheral port connected to the host via the USB interface:
      iv.  The device specifier must be expanded with the serial ID of the device in the command line. This is accomplished with the keyword --SN. Execute the following:
      v.   Command for device ID to the device port as shown above with the string is as follows:

.. code:: shell

    ./script/boot.py --device /dev/ttyUSB2 --SN 15100010 --reset=evk42_bl bins/iperf3.elf ssid=InnoPhase passphrase=Inno@9070
...
Storage Data and File: Read/Write
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
E&D - Storage Data & File: Read/Write
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
.. code:: shell
    ...
    python script\\flash.py [device] **to_json** <user_specify_path>\\<user_spefcify_name>.json
