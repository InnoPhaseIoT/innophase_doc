...
+-----------------------------------------------------------------------+
| int MQTTNetworkConnect_Tls(MQTTNetwork \*n, char \*host, int port,    |
| ssl_wrap_cfg_t \*cfg);                                                |
+=======================================================================+
+-----------------------------------------------------------------------+
...
value num_conn=1. By using this boot argument, user can create a maximum
of two connections.
...
6. Set transport_mode=5 for websocket server authentication. This is an
   encrypted connection and will require the server certificate.
...
| h= wcm_create(NULL);                                                  |
|                                                                       |
| rval = wifi_connect_to_network(&h, WCM_CONN_WAIT_INFINATE,            |
| &wcm_connected);                                                      |
|                                                                       |
| if(rval < 0) {                                                        |
|                                                                       |
...
|                                                                       |
| if(m->transport == APP_MQTT_TM_TLS \|\| m->transport ==               |
...
| /\*Client certificate*/                                               |
|                                                                       |
| m->cfg.client_cert.buf = utils_file_get(m->client_cert,               |
| &m->cfg.client_cert.len);                                             |
|                                                                       |
| if(m->cfg.client_cert.buf == NULL && bargs->transport_mode ==         |
| TLS_CLIENT_VERFIY){                                                   |
|                                                                       |
...
| /\*Client key*/                                                       |
|                                                                       |
| m->cfg.client_key.buf = utils_file_get(m->client_key,                 |
| &m->cfg.client_key.len);                                              |
|                                                                       |
| if(m->cfg.client_key.buf == NULL && bargs->transport_mode ==          |
| TLS_CLIENT_VERFIY){                                                   |
|                                                                       |
...
| }                                                                     |
|                                                                       |
| snprintf(m->client_id, MAX_MQTT_CLIENT_ID_LEN, "T2\_%s\_%d",buf,      |
| conn_num);                                                            |
|                                                                       |
| os_printf("\\n%s:%d, size = %d", \__FUNCTION\_\_, \__LINE\_\_,        |
| sizeof(m->publish_topic));                                            |
|                                                                       |
| snprintf(m->publish_topic, MAX_TOPIC_LEN, "%s%s\_%d",                 |
|                                                                       |
| m->client_id,"/pt2", conn_num);                                       |
|                                                                       |
| snprintf(m->subscribe_topic, MAX_TOPIC_LEN, "%s%s\_%d",               |
|                                                                       |
...
+-----------------------------------------------------------------------+
| MQTTClientInit(cn->mqtt_client, cn->mqtt_network, 15*1000,            |
| cn->sendbuf,                                                          |
|                                                                       |
...
|                                                                       |
| os_printf("\\nMalloc Fail @%s:%d", \__FUNCTION\_\_,                   |
|                                                                       |
| \__LINE\_\_);                                                         |
|                                                                       |
| return -1;                                                            |
...
|                                                                       |
| ret = MQTTNetworkConnect(mqtt_network, (char \*)param.cloud_url,      |
| param.cloud_port);                                                    |
|                                                                       |
| if (ret != 0) {                                                       |
|                                                                       |
...
| }                                                                     |
|                                                                       |
| }else if((param.transport == APP_MQTT_TRANSPORT_TLS) \|\|             |
| (param.transport == APP_MQTT_TRANSPORT_TLS_NO_CERT_VERIFY)){          |
|                                                                       |
| /\*Secured MQTT*/                                                     |
|                                                                       |
| MQTTNetworkInit_Tls(mqtt_network);                                    |
|                                                                       |
| ret = MQTTNetworkConnect_Tls(mqtt_network,                            |
| (char\*)param.cloud_url,param.cloud_port,&cfg);                       |
|                                                                       |
| if (ret < 0) {                                                        |
|                                                                       |
...
+-----------------------------------------------------------------------+
| MQTTClientInit(mqtt_client, mqtt_network, 15*1000, sendbuf,           |
| STW_MQTT_SEND_BUF_SIZE, readbuf, STW_MQTT_READ_BUF_SIZE);             |
|                                                                       |
| MQTTPacket_connectData data = MQTTPacket_connectData_initializer;     |
|                                                                       |
| data.willFlag = 0;                                                    |
...
| <Password> -t T2\_<mac id of T2>/subscribe                            |
+=======================================================================+
+-----------------------------------------------------------------------+
...
+-----------------------------------------------------------------------+
| mosquitto_pub.exe -h test.mosquitto.org -p 1883 -u <user name> -P     |
| <Password> -t T2\_<mac id of T2>/publisher -m “Message to Publish”    |
+=======================================================================+
+-----------------------------------------------------------------------+
...
Refer to section 10 to access the validity of the certificates.

Installing and Running Mosquitto MQTT Tool
------------------------------------------
...
Ensure the note in section 7.6 is followed and the binary is generated.
...
1. Certificates need to be flashed for secure MQTT. In case of
   ...
   sdk_x.y/examples/mqtt/certs/data directory.

..

   **Note**: sdk_x.y refers to the SDK version. For example: sdk_2.5.
...
2. Program mqtt.elf (*sdk_x.y/examples/mqtt/bin*) using the Download
   Tool.
...
| name >,cloud_usr_psw=<password>,mqtt_no_poll=1, num_conn=1            |
+=======================================================================+
+-----------------------------------------------------------------------+
...
+-----------------------------------------------------------------------+
| cloud_url=test.mosquitto.org,cloud_port=8883,                         |
| cloud_usr_name=<username>,cloud_usr_psw=<password>,transport_mode=    |
| 1,pub_qos=1,sub_qos=1,ca_cert=/data/mosquitto.org.crt,mqtt_no_poll=1, |
| num_conn=1                                                            |
+=======================================================================+
+-----------------------------------------------------------------------+
...
+-----------------------------------------------------------------------+
| cloud_url=test.mosquitto.org,cloud_port=8884,                         |
| cloud_usr_name=<username>,cloud_usr_psw=<password>,,tra               |
| nsport_mode=2,pub_qos=1,sub_qos=1,ca_cert=/data/mosquitto.org.crt,cli |
...
+-----------------------------------------------------------------------+
| cloud_url=test.mosquitto.                                             |
| org,cloud_port=8081,cloud_usr_name=innophase,cloud_usr_psw=innophase, |
| mqtt_no_poll=1,num_conn=1,transport_mode=5,pub_qos=1, sub_qos=1,      |
| websock_url=ws://test.mosquitto.org/mqtt,                             |
| ca_cert=/data/mosqu                                                   |
| itto.org.crt,client_cert=/data/client.crt,client_key=/data/client.key |
+=======================================================================+
+-----------------------------------------------------------------------+
...
+-----------------------------------------------------------------------+
| cloud_url=test.mosquitto.org,cloud_port=8091,cloud_usr_name=<         |
| username>,cloud_usr_psw=<password>,mqtt_no_poll=1,num_conn=1,transpor |
| t_mode=6,pub_qos=1,sub_qos=1,websock_url=ws://test.mosquitto.org/mqtt |
+=======================================================================+
+-----------------------------------------------------------------------+
...
| cloud_usr_psw=innophase pub_qos=2 sub_qos=1 mqtt_no_poll=1 num_conn=1 |
| np_conf_path=/data/nprofile.json ssid=inno_test passphrase=12345678   |
|                                                                       |
| \*MQTT example app v1.0\*                                             |
|                                                                       |
| addr e0:69:3a:00:0a:66                                                |
|                                                                       |
| Connecting to added network : inno_test                               |
|                                                                       |
| [0.807,932] CONNECT:04:d1:3a:b2:48:63 Channel:6 rssi:-47 dBm          |
|                                                                       |
| wcm_notify_cb to App Layer - WCM_NOTIFY_MSG_LINK_UP                   |
|                                                                       |
| wcm_notify_cb to App Layer - WCM_NOTIFY_MSG_ADDRESS                   |
|                                                                       |
| [0.948,054] MYIP 192.168.43.164                                       |
|                                                                       |
| [0.948,217] IPv6 [fe80::e269:3aff:fe00:a66]-link                      |
|                                                                       |
...
| FLASH:PNWWWWWAEBuild $Id: git-ba65998b7 $                             |
|                                                                       |
| cloud_url=test.mosquitto.org cloud_port=8883 cloud_usr_name=innophase |
| cloud_usr_psw=innophase transport_mode=1                              |
| ca_cert=/data/mosquitto.org.crt pub_qos=2 sub_qos=1 mqtt_no_poll=1    |
| num_conn=1 np_conf_path=/data/nprofile.json ssid=inno_test            |
| passphrase=12345678                                                   |
|                                                                       |
| \*MQTT example app v1.0\*                                             |
|                                                                       |
...
| Connecting to added network : inno_test                               |
|                                                                       |
| [0.760,784] CONNECT:04:d1:3a:b2:48:63 Channel:6 rssi:-44 dBm          |
|                                                                       |
| wcm_notify_cb to App Layer - WCM_NOTIFY_MSG_LINK_UP                   |
|                                                                       |
| wcm_notify_cb to App Layer - WCM_NOTIFY_MSG_ADDRESS                   |
|                                                                       |
| [2.744,711] MYIP 192.168.43.164                                       |
|                                                                       |
| [2.744,758] IPv6 [fe80::e269:3aff:fe00:a66]-link                      |
|                                                                       |
| wcm_notify_cb to App Layer - WCM_NOTIFY_MSG_CONNECTED                 |
|                                                                       |
| Connected to added network : inno_test                                |
...
| ROM yoda-h0-rom-16-0-gd5a8e586                                        |
|                                                                       |
| FLASH:PNWWWWWAEBuild $Id: git-76fbc149e $                             |
|                                                                       |
| cloud_url=test.mosquitto.org cloud_port=8080 cloud_usr_name=innophase |
| cloud_usr_psw=innophase mqtt_no_poll=1 num_conn=1 transport_mode=4    |
| websock_url=ws://test.mosquitto.org/mqtt                              |
| np_conf_path=/data/nprofile.json ssid=Inno_AP passphrase=inno1234     |
|                                                                       |
| $App:git-0dceba83                                                     |
|                                                                       |
| SDK Ver: sdk_2.6                                                      |
...
| addr e0:69:3a:00:0a:66                                                |
|                                                                       |
| Connecting to added network : Inno_AP                                 |
|                                                                       |
| [0.812,590] CONNECT:96:6a:1b:0d:62:e4 Channel:1 rssi:-25 dBm          |
|                                                                       |
| wcm_notify_cb to App Layer - WCM_NOTIFY_MSG_LINK_UP                   |
|                                                                       |
| app_wifi_status_cb: status = 200wcm_notify_cb to App Layer -          |
...
|                                                                       |
| Connected to added network : Inno_AP                                  |
|                                                                       |
...
|                                                                       |
| cloud_url=172.16.16.30 cloud_port=9001 cloud_usr_name=innophase       |
| cloud_usr_psw=innophase mqtt_no_poll=1 num_conn=1 transport_mode=5    |
| websock_url=ws://172.16.16.30/mqtt ca_cert=/data/ca.crt               |
| client_cert=/data/client.crt client_key=/data/client.key              |
...
| MQTT Example App                                                      |
|                                                                       |
| addr 02:03:04:84:ea:1d                                                |
|                                                                       |
| Connecting to added network : Xiaomi_Ax6000_iop                       |
|                                                                       |
| [1.987,190] CONNECT:d4:da:21:54:d3:c6 Channel:1 rssi:-20 dBm          |
|                                                                       |
| wcm_notify_callback :WCM_NOTIFY_MSG_LINK_UP                           |
|                                                                       |
| app_wifi_status_cb: status = 757544wcm_notify_callback                |
| :CM_NOTIFY_MSG_ADDRESS                                                |
|                                                                       |
| app_wifi_status_cb: status = 744624[2.740,120] MYIP 192.168.31.189    |
|                                                                       |
| [2.740,166] IPv6 [fe80::3:4ff:fe84:ea1d]-link                         |
|                                                                       |
| wcm_notify_callback :\_NOTIFY_MSG_CONNECTED                           |
|                                                                       |
| app_wifi_status_cb: status = 757480                                   |
|                                                                       |
| Connected to added network : Xiaomi_Ax6000_iop                        |
|                                                                       |
| MQTTNoPollInit:712 - initdone = 0                                     |
|                                                                       |
| MQTTRun_NoPollThread: 658mac id:23484ea1d                             |
|                                                                       |
| ------------------------------------------------------                |
|                                                                       |
| T2 MQTT Client id : T2_23484ea1d_1                                    |
|                                                                       |
| T2 MQTT publish topic : T2_23484ea1d_1/pt2_1                          |
|                                                                       |
| T2 MQTT subscribe topic: T2_23484ea1d_1/st2_1                         |
|                                                                       |
| T2 LWT topic : will_con1                                              |
|                                                                       |
| --------------------------------------------------------              |
...
Figure : Talaria TWO’s Published messages
...
Figure : The published message will be displayed on Talaria TWO’s
console
...
+-----------------------------------------------------------------------+
| mosquitto_sub.exe -h Innophase-SYNEM2043 -p 1883 -u innophase -P      |
| innophase -t will_con1                                                |
+=======================================================================+
+-----------------------------------------------------------------------+
...
Figure : MQTT client subscriber for LWT Topic
...
Figure : LWT publish message published to all the clients
...
Figure : MQTTBox Extension settings
...
Figure : MQTTBox Extension publish subscribe window
...
Figure : Published message over the Talaria TWO console
...
Figure : mosquito configuration file
...
| MQTT server> -u <user name> -P <Password> -t T2\_<mac id of           |
| T2_1/pt2_1                                                            |
+=======================================================================+
+-----------------------------------------------------------------------+
...
7. Program the mqtt_non_secured.elf along with the following bootargs
   using the Download Tool (refer steps from section 8.2 step 2).

+-----------------------------------------------------------------------+
| cloud_url= < IP address of the host PC running the local MQTT         |
| server>,cloud_port=1883,cloud_usr_name=<user name                     |
| >,cloud_usr_psw=<password>,mqtt_no_poll=1, num_conn =1                |
+=======================================================================+
+-----------------------------------------------------------------------+
...
| FLASH:PNWWWWWAEBuild $Id: git-ba65998b7 $                             |
|                                                                       |
| cloud_url=Innophase-SYNEM2043 cloud_port=1883                         |
| cloud_usr_name=innophase cloud_usr_psw=innophase pub_qos=2 sub_qos=1  |
| mqtt_no_poll=1 num_conn =1 np_conf_path=/data/nprofile.json           |
...
+-----------------------------------------------------------------------+
| mosquitto_sub.exe -h <IP addr of local machine > -u innophase -P      |
| innophase -t T2_e0693a0a66_1/pt2_1                                    |
+=======================================================================+
+-----------------------------------------------------------------------+
...
+-----------------------------------------------------------------------+
| mosquitto_pub.exe -h 192.168.43.3 -p 1883 -u innophase -P innophase   |
| -t T2_e0693a0eba_1/st2_1 -m "Sending Data to T2"                      |
+=======================================================================+
+-----------------------------------------------------------------------+
...
..
   ...
   Figure : Environment variables - User variables
...
Figure : Edit Environment Variable
...
..
   ...
   Figure : Edit Environment Variables
...
Figure : Modified Environment Variables
...
..
   ...
   Figure : CA Certificate generation
...
..
   ...
   Figure : Server certificate generation
...
Figure : Output - netstat command
   ...
   Figure : Newly generated certificates to file system

   |image36|

   Figure : Certificates available in File System

Programing Talaria TWO 
~~~~~~~~~~~~~~~~~~~~~~~

1. Program mqtt.elf (*sdk_x.y/examples/mqtt/bin*)using the Download
      Tool.
...
| pub_qos=2,sub_qos=1,mqtt_no_poll=1, num_conn=1                        |
+=======================================================================+
+-----------------------------------------------------------------------+
...
| Build $Id: git-defd1fcaf $                                            |
|                                                                       |
| cloud_url=172.16.16.30 cloud_port=8884 cloud_usr_name=innophase       |
...
| wcm_notify_callback :WCM_NOTIFY_MSG_LINK_UP                           |
|                                                                       |
| app_wifi_status_cb: status = 757536wcm_notify_callback                |
| :CM_NOTIFY_MSG_ADDRESS                                                |
|                                                                       |
...
| MQTTNoPollInit:712 - initdone = 0                                     |
|                                                                       |
| MQTTRun_NoPollThread: 658mac id:234c2d50                              |
|                                                                       |
| ------------------------------------------------------                |
|                                                                       |
| T2 MQTT Client id : T2_234c2d50_1                                     |
|                                                                       |
...
Figure : Secure MQTT - server – Console
...
Figure : Certificates verification
...
.. |image15| image:: media/image17.png
   :width: 5.90551in
   :height: 2.95736in
.. |image16| image:: media/image4.png
   :width: 5.90551in
   :height: 0.66995in
...
.. |image23| image:: media/image23.png
   :width: 3.14961in
   :height: 3.47452in
.. |image24| image:: media/image24.tmp
   :width: 5.51181in
   :height: 1.35733in
.. |image25| image:: media/image25.png
   :width: 5.51181in
   :height: 2.38165in
.. |image26| image:: media/image26.tmp
   :width: 5.51181in
   :height: 1.96044in
.. |image27| image:: media/image27.png
   :width: 5.51181in
   :height: 3.26837in
.. |image28| image:: media/image28.tmp
   :width: 5.51181in
   :height: 0.65205in
.. |image29| image:: media/image29.tmp
   :width: 5.51181in
   :height: 0.91349in
.. |image30| image:: media/image30.png
   :width: 5.51181in
   :height: 3.19275in
.. |image31| image:: media/image31.tmp
   :width: 5.51181in
   :height: 0.49097in
.. |image32| image:: media/image32.tmp
   :width: 5.90551in
   :height: 2.7272in
.. |image33| image:: media/image33.tmp
   :width: 5.51181in
   :height: 1.55536in
